# CGMusicalComposition: Algorithmic Music Engraving & Acoustic Simulation Engine for Concert Band
...
* **14-Instrument Benchmark Suite**: Features every acoustic instrument playing an individual solo medley, complete with neural voice narration (`benchmark_instruments_showcase_narrated.wav`).
...
| **Moonlight Samba**<br>*Beethoven Op. 27 No. 2 recast in Latin Bossa* | C minor | 116 BPM | [moonlight_samba.pdf](file:///C:/dev/CGMusicalComposition/moonlight_samba.pdf) | [moonlight_samba.wav](file:///C:/dev/CGMusicalComposition/moonlight_samba.wav) | [moonlight_samba.mid](file:///C:/dev/CGMusicalComposition/moonlight_samba.mid) |
...
| **14** | **Concert Percussion** | Snare, Bass, Cymbals, Toms | Drum | Dynamic battery drive, military rolls, crashing accents |
...
### 2. Acoustic Reverb DSP
Raw MIDI synthesis sounds clinical and robotic. All FluidSynth renders apply calibrated acoustic hall spatialization:
```bash
fluidsynth.exe -F output.wav \
...
### 3. Neural Voice Narration Pipeline (`build_showcase_audio.py`)
...
4. Concatentates speech labels and instrument solos with natural reverberant room decay and peak normalizes to $-0.3\text{ dBFS}$.
...
* **Python** (v3.10+): With `soundfile` and `numpy` installed

### Engrave a Conductor Score
```bash
lilypond a_soix_medley.ly
```
*Outputs:* `a_soix_medley.pdf` (Conductor Score) and `a_soix_medley.mid` (Multi-Track MIDI).

### Synthesize Master Audio
```bash
fluidsynth -F a_soix_medley.wav \
  -o synth.reverb.active=1 \
  -o synth.reverb.room-size=0.75 \
  -o synth.reverb.damp=0.25 \
  -o synth.reverb.width=0.85 \
...
`#concert-band` `#lilypond` `#fluidsynth` `#sheet-music` `#audio-synthesis` `#wind-ensemble` `#brass-band` `#percussion-solo` `#toms-solo` `#marimba` `#midi` `#wisconsin-rapids-city-band` `#john-daniel-dondlinger`
